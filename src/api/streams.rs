...
fn setup_passthrough_prototype(
    scope: &mut v8::HandleScope,
    template: &v8::FunctionTemplate,
) -> Result<(), anyhow::Error> {
    let proto = template.prototype_template(scope);
...
fn readable_constructor(
    ...
    readable_state.set(
        scope,
        high_water_mark_key.into(),
        high_water_mark_val.into(),
    );
