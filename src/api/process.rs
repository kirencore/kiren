...
fn process_on(
    ...
    if args.length() < 2 {
        let error = v8::String::new(scope, "process.on() requires event name and listener function").unwrap();
    ...
    if !event_arg.is_string() || !listener_arg.is_function() {
        let error = v8::String::new(scope, "process.on() requires string event name and function listener").unwrap();
        let exception = v8::Exception::type_error(scope, error);
        scope.throw_exception(exception);
        return;
    }
...
fn process_kill(
    ...
    let signal = if args.length() > 1 {
        ...
        signal_arg.to_string(scope).unwrap().to_rust_string_lossy(scope)
    } else {
        "SIGTERM".to_string()
    };
...
fn process_chdir(
    ...
    if let Ok(path_str) = v8::Local::<v8::String>::try_from(path_arg) {
        ...
        if let Err(_) = std::env::set_current_dir(&path) {
            let error_msg = v8::String::new(scope, &format!("Failed to change directory to: {}", path)).unwrap();
