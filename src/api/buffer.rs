...
pub fn initialize_buffer_api(scope: &mut v8::HandleScope, global: v8::Local<v8::Object>) -> Result<(), anyhow::Error> {
...
fn buffer_constructor(
    ...
    if args.length() == 0 {
        let msg = v8::String::new(scope, "Buffer constructor requires at least one argument").unwrap();
    ...
    if arg0.is_number() {
        ...
        let encoding = if args.length() > 1 {
            args.get(1).to_string(scope).unwrap().to_rust_string_lossy(scope)
        } else {
            "utf8".to_string()
        };
        ...
        let data = match encoding.as_str() {
            ...
            "ascii" => {
                ...
                s.chars().take_while(|&c| c.is_ascii()).map(|c| c as u8).collect()
            },
            "base64" => {
                match base64::engine::general_purpose::STANDARD.decode(string_val.to_rust_string_lossy(scope)) {
                    Ok(decoded) => decoded,
                    Err(_) => {
            ...
            _ => {
                let msg = v8::String::new(scope, &format!("Unknown encoding: {}", encoding)).unwrap();
                let exception = v8::Exception::type_error(scope, msg);
                scope.throw_exception(exception);
                return;
            }
        ...
        let slice = unsafe { 
            std::slice::from_raw_parts_mut(backing_store.data().unwrap().as_ptr() as *mut u8, data.len()) 
        };
        ...
        if arg0.is_uint8_array() {
            ...
            let dest_slice = unsafe { 
                std::slice::from_raw_parts_mut(dest_backing_store.data().unwrap().as_ptr() as *mut u8, length) 
            };
            
            let src_backing_store = typed_array.buffer(scope).unwrap().get_backing_store();
            let src_slice = unsafe { 
                std::slice::from_raw_parts(
                    (src_backing_store.data().unwrap().as_ptr() as *const u8).add(typed_array.byte_offset()),
                    length
                ) 
            };
            ...
            let backing_store = array_buffer.get_backing_store();
            let slice = unsafe { 
                std::slice::from_raw_parts_mut(backing_store.data().unwrap().as_ptr() as *mut u8, length) 
            };
...
fn buffer_concat(
    ...
    let target_size = if args.length() > 1 {
        std::cmp::min(args.get(1).number_value(scope).unwrap_or(total_size as f64) as usize, total_size)
    } else {
        total_size
    };
    ...
    let dest_backing_store = array_buffer.get_backing_store();
    let dest_slice = unsafe { 
        std::slice::from_raw_parts_mut(dest_backing_store.data().unwrap().as_ptr() as *mut u8, target_size) 
    };
    
    let mut offset = 0;
    for buffer in buffers {
        if offset >= target_size { break; }
...
fn buffer_to_string(
    ...
    if this.is_uint8_array() {
        ...
        let end = if args.length() > 2 {
            args.get(2).number_value(scope).unwrap_or(buffer.length() as f64) as usize
        } else {
            buffer.length()
        };
        ...
        let slice = unsafe { 
            std::slice::from_raw_parts(
                (backing_store.data().unwrap().as_ptr() as *const u8).add(buffer.byte_offset() + start),
...
fn buffer_slice(
    ...
    if this.is_uint8_array() {
        ...
        let end = if args.length() > 1 {
            args.get(1).number_value(scope).unwrap_or(buffer.length() as f64) as usize
        } else {
            buffer.length()
        };
        ...
        let src_backing_store = buffer.buffer(scope).unwrap().get_backing_store();
        let dest_slice = unsafe { 
            std::slice::from_raw_parts_mut(dest_backing_store.data().unwrap().as_ptr() as *mut u8, slice_length) 
        };
        let src_slice = unsafe { 
            std::slice::from_raw_parts(
                (src_backing_store.data().unwrap().as_ptr() as *const u8).add(buffer.byte_offset() + start),
                slice_length
            ) 
        };
        dest_slice.copy_from_slice(src_slice);
...
fn buffer_write(
    ...
    if this.is_uint8_array() {
        ...
        let length = if args.length() > 2 {
            args.get(2).number_value(scope).unwrap_or((buffer.length() - offset) as f64) as usize
        } else {
            buffer.length() - offset
        };
        
        let encoding = if args.length() > 3 {
            args.get(3).to_string(scope).unwrap().to_rust_string_lossy(scope)
        } else {
            "utf8".to_string()
        };
        ...
        let slice = unsafe { 
            std::slice::from_raw_parts_mut(
                (backing_store.data().unwrap().as_ptr() as *mut u8).add(buffer.byte_offset() + offset),
                write_length
            ) 
        };
