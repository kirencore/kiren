...
pub fn decodeFrame(data: []const u8) !Frame {
    ...
    if (masked) {
        ...
        payload = unmasked[0..payload.len];
    }
...
pub fn encodeFrame(data: []const u8, opcode: Opcode, fin: bool) [4096]u8 {
...
pub fn handleWebSocket(allocator: Allocator, stream: net.Stream, request_data: []const u8) !void {
    ...
    while (true) {
        const bytes_read = stream.read(&buf) catch break;
        if (bytes_read == 0) break;

        const frame = decodeFrame(buf[0..bytes_read]) catch break;
        ...
        switch (frame.opcode) {
            ...
            .ping => {
                // Send pong
                const pong = encodeFrame(frame.payload, .pong, true);
