...
// Global test state
static TEST_STATE: Lazy<Arc<Mutex<TestState>>> =
    Lazy::new(|| Arc::new(Mutex::new(TestState::new())));
...
fn add_test_functions(scope: &mut v8::HandleScope, global: &v8::Local<v8::Object>) -> Result<()> {
    // describe(name, fn)
...
fn add_expect_api(scope: &mut v8::HandleScope, global: &v8::Local<v8::Object>) -> Result<()> {
...
fn assert_equal(
    ...
    if args.length() < 2 {
        let error =
            v8::String::new(scope, "assert.equal() requires actual and expected values").unwrap();
        let exception = v8::Exception::type_error(scope, error);
        scope.throw_exception(exception);
        return;
    }
...
fn assert_not_equal(
    ...
    if args.length() < 2 {
        let error = v8::String::new(
            scope,
            "assert.notEqual() requires actual and expected values",
        )
        .unwrap();
        let exception = v8::Exception::type_error(scope, error);
        scope.throw_exception(exception);
    ...
    if actual.strict_equals(expected) {
        ...
        let error_msg = format!(
            "Expected values to be different, but both were '{}'",
            actual_str
        );
        let error = v8::String::new(scope, &error_msg).unwrap();
        let exception = v8::Exception::error(scope, error);
