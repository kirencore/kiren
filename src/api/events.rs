...
pub fn initialize_events_api(scope: &mut v8::HandleScope, global: v8::Local<v8::Object>) -> Result<(), anyhow::Error> {
...
fn event_on(
    ...
    if args.length() < 2 {
        let msg = v8::String::new(scope, "EventEmitter.on() requires an event name and listener").unwrap();
    ...
    let this = args.this();
    let event_name = args.get(0).to_string(scope).unwrap().to_rust_string_lossy(scope);
...
fn event_once(
    ...
    if args.length() < 2 {
        let msg = v8::String::new(scope, "EventEmitter.once() requires an event name and listener").unwrap();
        let exception = v8::Exception::type_error(scope, msg);
        scope.throw_exception(exception);
        return;
    }
...
fn event_off(
    ...
    if args.length() < 2 {
        let msg = v8::String::new(scope, "EventEmitter.off() requires an event name and listener").unwrap();
        let exception = v8::Exception::type_error(scope, msg);
        scope.throw_exception(exception);
        return;
    }
...
fn event_listeners(
    ...
    if args.length() < 1 {
        let msg = v8::String::new(scope, "EventEmitter.listeners() requires an event name").unwrap();
        let exception = v8::Exception::type_error(scope, msg);
        scope.throw_exception(exception);
        return;
    }
...
fn event_listener_count(
    ...
    if args.length() < 1 {
        let msg = v8::String::new(scope, "EventEmitter.listenerCount() requires an event name").unwrap();
        let exception = v8::Exception::type_error(scope, msg);
        scope.throw_exception(exception);
        return;
    }
...
fn event_names(
    ...
    // Get property names
    let property_names = events_obj.get_own_property_names(scope, Default::default()).unwrap();
...
fn event_set_max_listeners(
    ...
    if args.length() < 1 {
        let msg = v8::String::new(scope, "EventEmitter.setMaxListeners() requires a number").unwrap();
        let exception = v8::Exception::type_error(scope, msg);
        scope.throw_exception(exception);
        return;
    }
...
fn check_max_listeners(scope: &mut v8::HandleScope, this: v8::Local<v8::Object>, event_name: &str, count: u32) {
    let max_listeners_key = v8::String::new(scope, "_maxListeners").unwrap();
    let max_listeners_val = this.get(scope, max_listeners_key.into()).unwrap();
