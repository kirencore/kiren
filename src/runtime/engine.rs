use crate::api::{
    buffer, crypto, environment, events, fetch, filesystem, filesystem_improved, http, process,
    process_improved, streams, test, timers,
};
...
impl Engine {
    ...
    fn transform_static_imports(&self, source: &str) -> String {
        ...
        transformed = NAMED_IMPORT_REGEX
            .replace_all(&transformed, |caps: &regex::Captures| {
                has_imports = true;
                format!(
                    "const {{ {} }} = await __kiren_import('{}');",
                    &caps[1], &caps[2]
                )
            })
            .to_string();
        ...
        transformed = DEFAULT_IMPORT_REGEX
            .replace_all(&transformed, |caps: &regex::Captures| {
                has_imports = true;
                format!(
                    "const {} = (await __kiren_import('{}')).default;",
                    &caps[1], &caps[2]
                )
            })
            .to_string();
...
static NAMED_IMPORT_REGEX: Lazy<regex::Regex> =
    Lazy::new(|| regex::Regex::new(r#"import\s+\{([^}]+)\}\s+from\s+['"]([^'"]+)['"];?"#).unwrap());

static DEFAULT_IMPORT_REGEX: Lazy<regex::Regex> =
    Lazy::new(|| regex::Regex::new(r#"import\s+(\w+)\s+from\s+['"]([^'"]+)['"];?"#).unwrap());
