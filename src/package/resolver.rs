...
impl PackageResolver {
    ...
    pub fn resolve_version(
        ...
        match spec {
            ...
            VersionSpec::Latest => self.get_latest_version(available_versions),
            VersionSpec::Range(range) => self.resolve_version_range(range, available_versions),
            VersionSpec::Tag(tag) => {
    ...
    fn parse_version_spec(version_str: &str) -> Result<VersionSpec> {
        match version_str {
            ...
            v if v.chars().next().unwrap_or('0').is_ascii_digit() => {
                Ok(VersionSpec::Exact(v.to_string()))
            }
